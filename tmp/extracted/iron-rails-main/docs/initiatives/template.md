# Initiative: [Title - Brief Description of the Initiative]

**Status:** Proposed | Active | Complete
**Created:** YYYY-MM-DD
**Owner:** [Name/Team]
**Priority:** High | Medium | Low

---

## 1. Objective

[Clear, single-paragraph statement of what this initiative aims to achieve. Should answer: What are we doing and why does it matter?]

---

## 2. Success Criteria

[Define measurable outcomes. Each criterion should be a checkbox that can be objectively verified.]

- [ ] Criterion 1 (specific, measurable)
- [ ] Criterion 2 (specific, measurable)
- [ ] Criterion 3 (specific, measurable)

---

## 3. Scope

### In Scope

- Item 1
- Item 2

### Out of Scope

- Item 1
- Item 2

---

## 4. Phases & Tasks

[Organize work into logical phases. Use checkboxes for trackable progress.]

### Phase 1: [Phase Name]

- [ ] Task 1 with specific deliverable
- [ ] Task 2 with specific deliverable

### Phase 2: [Phase Name]

- [ ] Task 1 with specific deliverable
- [ ] Task 2 with specific deliverable

---

## 5. Risks and Mitigation

[Identify potential risks and how to address them.]

| Risk | Likelihood | Mitigation Strategy |
| :--- | :--- | :--- |
| [Example: Scope creep] | Medium | [Fixed scope, defer non-critical items] |
| [Example: Technical complexity] | Low | [Spike work in Phase 1, adjust if needed] |

---

## 6. Related Documentation

[Cross-reference relevant files and documentation.]

- Link to GDD Document
- Link to Technical Analysis Document

---

## 7. Documentation Standards

All documentation related to this initiative must adhere to the standards outlined in the [documentation standards guide](./documentation-standards.md).
